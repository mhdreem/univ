...
{
    public class StatusService :GenericAppService<Status, StatusDto, int, StatusPagedAndSortedResultRequestDto, CreateStatusDto, UpdateStatusDto>, IStatusService
    {
        ...
        [HttpPost("api/app/status/filterdata")]
        public async Task<RespondDto>
                FilterDataAsync(StatusPagedAndSortedResultRequestDto input)
        {
            ...
            //Convert to DTOs
            var arrDtos = ObjectMapper.Map<List<Status>, List<StatusDto>>(arr);
            ...
            respond.Result = new PagedResultDto<StatusDto>(
                totalCount,
                arrDtos
            );
            return respond;
        ...
        [HttpPost("api/app/status/check")]
        public async Task<RespondDto>
       CheckAsync(CheckStatusDto input)
        {
            RespondDto respond = new RespondDto();
        ...
        [HttpPost("api/app/status/postdata")]
        public async Task<RespondDto> PostDataAsync(CreateStatusDto input)
        {
            ...
            RespondDto respond = new RespondDto();
            respond.IsSuccess = true;
            respond.Result = ObjectMapper.Map<Status, StatusDto>(newObj);
        ...
        [HttpPut("api/app/status/modifydata/{id}")]
        public async Task<RespondDto> ModifyDataAsync(int id, UpdateStatusDto input)
        {
            var obj = ObjectMapper.Map<UpdateStatusDto, Status>(input);
            ...
            RespondDto respond = new RespondDto();
            respond.IsSuccess = true;
            respond.Result = ObjectMapper.Map<Status, StatusDto>(obj);
            return respond;
        }
        ...
        [HttpDelete("api/app/status/removedata/{id}")]
